...
public class CCC2 {
		...
		for (int i = 0; i < m; i++) {
			...
			if (x == 1) {
				...
				while (true) {
					...
					//System.out.println((int)Math.sqrt(a[ans])-a[ans]);
	...
	static long build(int pos, int l, int r) {
	...
	static long update(int pos, int ind, int x) {
	...
	static long query(int pos, int ll, int rr) {
		int l = tree[pos].l, r = tree[pos].r;
		if (l > rr || r < ll) {
			return 0;
		} else if (l >= ll && r <= rr) {
