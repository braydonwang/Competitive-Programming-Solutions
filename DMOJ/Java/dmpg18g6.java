...
public class CCC2 {
	...
	public static void main(String[] args) throws IOException {
		...
		ArrayList<Integer> cl[] = new ArrayList[c+1];
		for (int i = 1; i <= n; i++) {
			st[i] = new ArrayList();
			v[i] = readInt();
		}
		...
		dis[1] = 0; q.add(new Edge(1,0)); vis[1] = true;
		while (!q.isEmpty()) {
			Edge cur = q.poll();
			if (cur.w > dis[cur.v]) {
				continue;
			}
			for (int clas: st[cur.v]) {
				for (int stud: cl[clas]) {
					long temp = Math.abs(v[cur.v] - v[stud]) + k;
					if (!vis[stud] && dis[stud] > dis[cur.v] + temp) {
						dis[stud] = dis[cur.v] + temp;
						q.add(new Edge(stud,dis[stud])); 
					}
				}
			}
		}
		for (int i = 1; i <= n; i++) {
			System.out.println(dis[i] == 1e12 ? -1 : dis[i]);
		}
	}
